...
**Format**: `src/assets/{variant}_{base_name_lowercase}_sprite.webp` (e.g. `gold_water_sprite.webp`), matching the existing decorative assets already in that folder.

**Multi-word / flavor base names**: many `base_name`s are multiple words (e.g. `Zero Point`, `Peeky Peely`, `Lootin' Llama`, `Vini Jr.`). `upload-sprite-images.js` matches a file to its `sprites` row by taking the first `_`-delimited segment as the variant (if it's a known variant) and the rest as the base name, then comparing with **all non-alphanumerics stripped and lowercased** on both sides. So the base portion of the filename may use `_` (or nothing) between words freely — pick whichever reads clean; only the alphanumeric characters have to match the DB `base_name`. Examples: `gold_zero_point_sprite.webp` → `Zero Point`; `gummy_peeky_peely_sprite.webp` → `Peeky Peely`; `galaxy_lootin_llama_sprite.webp` → `Lootin' Llama` (the apostrophe is stripped). The upload script warns and skips any file it can't match, so a normalization mismatch fails loudly rather than silently.

Verify real alpha transparency before use — `sips -g hasAlpha <file>` should report `yes`, and the alpha channel should have genuine variance (`min != max`), not just a flattened white/dark background that happens to carry an alpha flag. If a source image has a flattened solid background instead, chroma-key removal (Pillow, distance-from-background-color with a smooth edge falloff) works well for flat/solid backgrounds; soft gradients or glow edges need a wider low/high distance threshold or per-image tuning.
